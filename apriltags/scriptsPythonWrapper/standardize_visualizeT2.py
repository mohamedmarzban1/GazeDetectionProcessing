...
def sync_back_and_visualize(visualize_path, back_path, visualize_frames, backOffset):
    ...
    while row_counter < data_rows and frame_counter < frames:
        ...
        if detectionID == 2222:
            ...
            vis_info[0], vis_info[1], vis_info[2] = -vis_info[2], -vis_info[0], vis_info[1]

            output_str = str(frameNew)
...
def standardize_visualizeT2(HAT_BACK, OP_VIS, STD_VIS, backOffset=0):
    #args = get_args()

    visualize_frames = open(STD_VIS, 'w+') 
    #sync_back_and_visualize(args.visualize, args.back, visualize_frames)
    sync_back_and_visualize(OP_VIS, HAT_BACK, visualize_frames, backOffset)
